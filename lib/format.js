...
function convertToIcal(docs, lang) {
    ...
    var calInfo = config.wastetypes();
    var vEvent, momDate;
    // eslint-disable-next-line complexity
    _.each(docs, function(doc) {
        vEvent = iCal.addComponent('VEVENT');
        if (doc.area && doc.area === doc.zip) {
            vEvent.setSummary(
                trans.translate(calInfo[doc.waste_type].name) + ', ' + trans.translate('ZIP: $[1]', doc.zip)
            );
        } else if (doc.area) {
            vEvent.setSummary(
                trans.translate(calInfo[doc.waste_type].name) + ', ' + trans.translate('Area: $[1]', doc.area)
            );
        } else {
        ...
        } else if (doc.area && doc.area !== doc.zip) {
        ...
        if (doc.description) {
            ...
            vEvent.setDescription(trans.translate(calInfo[doc.waste_type].description));
        }
