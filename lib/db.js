...
function calendar(callback, params, sort, limit) {
    db.open();
    var dbParams = parseParams(params) || {};
    var sort = parseSort(sort) || {};

    db.calendar.find(dbParams, {_id: 0}).sort(sort).limit(limit, function(err, docs) {
        if (err) {
            callback(err);
            return;
        }
        ...
        _.map(docs, function(doc) {
            ...
            zurichDate = dateObj.clone().tz('Europe/Zurich');
...
function station(callback, params, sort, limit) {
    db.open();
    var dbParams = parseParams(params) || {};
    var sort = parseSort(sort) || {};

    db.station.find(dbParams, {_id: 0}).sort(sort).limit(limit, function(err, docs) {
        if (err) {
            callback(err);
            return;
        }
...
function parseParams(params) {
    ...
    if (undefined !== params['station']) {
        var regex = new RegExp(".*" + params['station'] + ".*");
