...
function addGeoRoutes(server) {
    server.route({
        ...
        config: {
            ...
            notes: 'Returns the GeoJSON of all Cargo and E-Tram stops',
            tags: ['api'],
            jsonp: 'callback'
        },
        handler: function (request, reply) {
            ...
            if (accept.types(jsonContentType, geoJsonContentType) === geoJsonContentType) {
                contentType = geoJsonContentType;
            } else {
                contentType = jsonContentType;
            }
            reply(content).header('Content-Type', contentType);
        }
    });
}
