...
const areaModel = Joi.object({
    ...
    result: Joi.array().items(areaItem).label('List of area entries')
}).label('Area result');

const errorModel = Joi.object({
    statusCode: Joi.number().label('HTTP Status Code'),
    error: Joi.string().label('Type of error'),
...
function addParamRoutes(server) {
    ...
    server.route({
        /* eslint-disable max-len */
        method: 'GET',
        path: '/api/parameter/areas',
        config: {
            description: 'Values for the `area` parameter',
            notes: 'Returns a list of string values for the `area` parameter (by region if the `region` query parameter is given).',
            tags: [ 'api', 'parameter' ],
            plugins: {
                'hapi-swagger': {
                    responses: {
                        200: { schema: areaModel },
                        400: { schema: errorModel}
                    }
                }
            },
