...
function parameterConfig(inputTypes) {
    if (types) {
        var types = Joi.string().valid(...inputTypes);
    } else {
        var types = Joi.string().valid(
            'cardboard',
            'cargotram',
            'etram',
            'organic',
    ...
    return {
        region: Joi.string().valid('zurich', 'thalwil', 'basel', 'stgallen').description('The region of this collection'),
