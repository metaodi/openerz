...
do
    ...
    echo "Generate ${r} CSV..."
    uv run $DIR/generate_from_config.py -c $DIR/config/regions/$r.yml -o $DIR/csv/$r/$r.csv --verbose
    uv run $DIR/sort_csv.py -i $DIR/csv/$r/$r.csv -o $DIR/csv/$r/$r.csv -s "col_date, waste_type"

    echo "Import ${r}_stationen.csv CSV..."
    uv run $DIR/update_database.py -f $DIR/csv/${r}/${r}_stationen.csv --table station --region $r 
    echo "Import ${r}.csv CSV..."
    uv run $DIR/update_database.py -f $DIR/csv/${r}/${r}.csv --table calendar --region $r 
done
